...
public class CharcoalBlock extends BlockItem {
	public CharcoalBlock() {
		super(BlockInit.CHARCOALBLOCK.get(), new BlockItem.Properties().group(TnCCoalWorks.TNCCOALWORKS_GROUP));
	}
	@Override
	@OnlyIn(Dist.CLIENT)
	public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
		...
		if (InputMappings.isKeyDown(Minecraft.getInstance().getMainWindow().getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
			...
			tooltip.add(new TranslationTextComponent("Cooks 72 items").mergeStyle(TextFormatting.BLUE));
		}
	}
	@Override
