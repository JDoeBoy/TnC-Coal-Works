...
public class PolCharcoalBlock extends BlockItem {
	public PolCharcoalBlock() {
		super(BlockInit.POLCHARCOALBLOCK.get(), new BlockItem.Properties().group(TnCCoalWorks.TNCCOALWORKS_GROUP));
	}
	@Override
	@OnlyIn(Dist.CLIENT)
	public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
		...
		if (InputMappings.isKeyDown(Minecraft.getInstance().getMainWindow().getHandle(), GLFW.GLFW_KEY_LEFT_SHIFT)) {
			...
			tooltip.add(new TranslationTextComponent("Cooks 288 items").mergeStyle(TextFormatting.BLUE));
		}
	}
	@Override
